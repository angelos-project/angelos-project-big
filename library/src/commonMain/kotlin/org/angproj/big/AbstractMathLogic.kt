...
public abstract class AbstractMathLogic: BufferAware {
    ...
    protected inline fun <reified R: Any> getIdx(
    ...
    protected inline fun <reified R: Any> getIdxL(
        mag: IntArray, sigNum: BigSigned, firstNonZero: Int, index: Int
    ): Long = getIdx<Unit>(mag, sigNum, firstNonZero, index).toLong() and 0xffffffffL

    protected inline fun <reified R: Any> getL(value: Int): Long = value.toLong() and 0xffffffffL

    protected inline fun <reified R: Any>setIdx(mag: IntArray, index: Int, num: Int): Unit = revSet<Unit>(mag, index, num)
    protected inline fun <reified R: Any>setIdxL(mag: IntArray, index: Int, num: Long): Unit = revSet<Unit>(mag, index, num.toInt())
    ...
    protected inline fun <reified R: Any> keep(value: ByteArray, sigNum: BigSigned): Int {
        ...
        return innerKeep<Unit>(keep, value.size)
    }
    ...
    protected inline fun <reified R: Any> keep(value: Binary, sigNum: BigSigned): Int {
        ...
        return innerKeep<Unit>(keep, value.limit)
    }
    ...
    protected inline fun <reified R: Any> innerKeep(keep: Int, size: Int): Int = when (keep) {
        -1 -> size
        else -> keep
    }
    ...
    protected inline fun <reified R: Any> stripLeadingZeros(value: ByteArray): IntArray {
        val keep = keep<Unit>(value, BigSigned.POSITIVE)
    ...
    protected inline fun <reified R: Any> stripLeadingZeros(value: Binary): IntArray {
        val keep = keep<Unit>(value, BigSigned.POSITIVE)
