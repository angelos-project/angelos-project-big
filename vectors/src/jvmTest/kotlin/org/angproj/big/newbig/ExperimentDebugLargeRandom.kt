...
class ExperimentDebugLargeRandom {
    ...
    private fun generateRandomBigInts(){
        shortPositive = internalOf(Random.nextBytes(192 / 8)).abs()
        shortNegative = internalOf(Random.nextBytes(192 / 8)).abs().negate()
        longPositive = internalOf(Random.nextBytes(256 / 8)).abs()
        longNegative = internalOf(Random.nextBytes(256 / 8)).abs().negate()
    }
